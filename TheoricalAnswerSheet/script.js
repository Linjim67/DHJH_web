...
const questionStates = {
    ...
    q24: { type: 'mixed', attempts: 0, maxPoints: 5, isCorrect: false, correctHash: { radio: "6f40fb56", text: "b709b7f3" } },
    q25: { type: 'mixed', attempts: 0, maxPoints: 5, isCorrect: false, correctHash: { radio: "3bf1d575", text: "7e93fd0f" } },
...
window.encryptAnswer = function (qId, answer) {
    ...
    // 💡【新增防呆】：如果您 HTML 的 ID 是寫 q231，這裡會自動把它變成 23-1 來對齊 C++
    if (/^\d{3}$/.test(cleanId)) {
        cleanId = cleanId.substring(0, 2) + "-" + cleanId.substring(2);
    }

    // 3. 組合鹽巴 (現在 cleanId 絕對會是 23-1 這種格式)
